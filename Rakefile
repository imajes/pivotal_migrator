...
Rake::Task["release"].enhance do
  ...
  system "open https://github.com/imajes/pivotal_migrator/releases"
end

task :verify_gemspec_files do
  git_files = `git ls-files -z`.split("\x0")
  gemspec_files = Gem::Specification.load("pivotal_migrator.gemspec").files.sort
  ignored_by_git = gemspec_files - git_files
  next if ignored_by_git.empty?

  raise <<~ERROR

    The `spec.files` specified in pivotal_migrator.gemspec include the following files
...
namespace :bump do
  ...
  task :ruby do
    replace_in_file "pivotal_migrator.gemspec", /ruby_version = .*">= (.*)"/ => RubyVersions.lowest
